...
- Clicking on window will now focus not just the last focused element, but also if there wasn't one, it will focus a control with `class="default"`, and if that doesn't exist, the first (tabbable) control, and if there's no (tabbable) controls, the window itself (specifically `$window.$content`). For tool windows it focuses the parent window.
- `$window.focus()` now actually focuses something, rather than just bringing the window to the top and making it appear active. For tool windows, it will focus the parent window. For app windows, it will focus the last focused control within the window, or else a control with `class="default"`, or else the first tabstop within the window, or else the window itself (specifically `$window.$content`).
